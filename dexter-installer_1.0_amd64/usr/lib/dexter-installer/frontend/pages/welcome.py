...
class WelcomePage:
    ...
    def __init__(self, app):
        ...
        # Título de la página
        title = Gtk.Label()
        title.set_markup("<span foreground='white' size='26000' weight='bold'>{}</span>".format(
        ...
        # Subtítulo
        subtitle = Gtk.Label()
        subtitle.set_markup("<span size='large'>{}</span>".format(
        ...
        info_label.set_markup("<span size='medium'>{}</span>".format(
        ...
        # Lista de aspectos a configurar
        points = [
            _("Idioma y ubicación regional"),
            _("Zona horaria"),
            _("Disposición del teclado"),
            _("Configuración de las particiones del disco"),
            _("Creación de una cuenta de usuario")
        ]
        
        for point in points:
            point_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
            point_box.set_margin_start(10)
            ...
            info_box.pack_start(point_box, False, False, 5)
        
        # Nota de recomendación
        note_label = Gtk.Label()
        note_label.set_markup("<span size='small' style='italic'>{}</span>".format(
